...
import sys
sys.path.append(os.path.abspath("/workspace/kevin_infographics/Qwen2VL"))
...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        model = PeftModel.from_pretrained(model, "/workspace/checkpoint-96", is_trainable=True).to("cuda")
    ...
    def _get_key_from_inputs(self, x):
        key1 = "jpg_0"
        key2 = "jpg_1"
        ele1 = x.get(key1, None)
        ele2 = x.get(key2, None)
        assert ele1 is not None, f"The key {key1} is not found in the input"
        assert ele2 is not None, f"The key {key2} is not found in the input"

        return [PIL.Image.open(BytesIO(ele1)).resize((448,448)), PIL.Image.open(BytesIO(ele2)).resize((448,448))]
    
    def get_human_pref(self, best_image_uid, image_0_uid, image_1_uid):
        if best_image_uid == image_0_uid:
            return 1
        elif best_image_uid == image_1_uid:
            return 2
        else:
            print(best_image_uid, image_0_uid, image_1_uid)
            print("error case")
            return "none"
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        images = []
        for x in inputs:
            imgs = self._get_key_from_inputs(x)
        ...
        solutions = [self.get_human_pref(x["best_image_uid"], x["image_0_uid"], x["image_1_uid"]) for x in inputs]
